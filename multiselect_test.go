...
func TestMultiSelectRender(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			strings.Join(
				[]string{
					fmt.Sprintf("%s Escolha suas opções: [Use as setas para mover, barra de espaço para selecionar, <direita> para todos, <esquerda> para nenhum, digite para filtrar]", defaultIcons().Question.Text),
		...
		},
		{
			...
			strings.Join(
				[]string{
					fmt.Sprintf("%s Pick your words:  [Use as setas para mover, barra de espaço para selecionar, <direita> para todos, <esquerda> para nenhum, digite para filtrar, %s para mais ajuda]", defaultIcons().Question.Text, string(defaultPromptConfig().HelpInput)),
					fmt.Sprintf("  %s  foo", defaultIcons().UnmarkedOption.Text),
					fmt.Sprintf("  %s  bar", defaultIcons().MarkedOption.Text),
					fmt.Sprintf("%s %s  baz", defaultIcons().SelectFocus.Text, defaultIcons().UnmarkedOption.Text),
					fmt.Sprintf("  %s  buz\n", defaultIcons().MarkedOption.Text),
	...
	tests := []PromptTest{
		{
			...
			func(c expectConsole) {
				c.ExpectString("Quais dias você prefere: [Use as setas para mover, barra de espaço para selecionar, <direita> para todos, <esquerda> para nenhum, digite para filtrar]")
				// Select Monday.
		...
		},
		{
			...
			func(c expectConsole) {
				c.ExpectString("Quais dias você prefere: [Use as setas para mover, barra de espaço para selecionar, <direita> para todos, <esquerda> para nenhum, digite para filtrar], ? for more help")
				c.Send("?")
		...
		},
		{
			...
			func(c expectConsole) {
				c.ExpectString("今天中午吃什么？  [Use as setas para mover, barra de espaço para selecionar, <direita> para todos, <esquerda> para nenhum, digite para filtrar]")
				// Filter down to 小炒肉.
				c.Send("小炒")
	...
	tests := []PromptTest{
		{
			...
			func(c expectConsole) {
				c.ExpectString("What color do you prefer:  [Use as setas para mover, barra de espaço para selecionar, <direita> para todos, <esquerda> para nenhum, digite para filtrar]")
